module Infiltrator exposing (..)

import Html exposing (..)
import Message exposing (Message)
import Style exposing (..)


patch_1_121 : List (Html Message)
patch_1_121 =
    [h2 [] [text "インフィルトレーター 1.121"],
     text "インフィルトレーターはフルリスペックされる。",
     text "インフィルトレーターはレルムアビリティーをリスペックされる。",
     text "スペックポイントが 2.5 から 2.9 になる。",
     text "レルムアビリティー Mastery of Magery と Wild Power を選択可能になる。",
     text "Vanish はレルムアビリティーではなく Stealth スペックに含まれる。",
     text "Mastery of Stealth はレルムアビリティーではなく Stealth スペックの Shadow Run になる。",
     text "アサシンはレルムアビリティー Determination と Strike Prediction を選択可能になる。",
     text "アサシンから Heightened Awareness, Blood Rage, Subtlety アビリティーが除去される。",
     text "Remedy は再使用 5 分となり HP を失うことはなくなる。",
     br [] [],
     text "Viper の値が変更になる。",
     ulist ["Level 1 - 10% から 5%",
            "Level 2 - 20% から 10%",
            "Level 3 - 35% から 20%",
            "Level 4 - 50% から 30%",
            "Level 5 - 75% から 40%"],
     secalb "Stealth (スペック)",
     text "新しく Shadow Run が追加される。",
     ulist ["Level 10 - Shadow Run I - instant cast - Permanent 継続 - 5% detection bonus",
            "Level 15 - Shadow Run II - instant cast - Permanent 継続 - 10% detection bonus",
            "Level 20 - Shadow Run III - instant cast - Permanent 継続 - 15% detection bonus",
            "Level 25 - Shadow Run IV - instant cast - Permanent 継続 - 20% detection bonus",
            "Level 30 - Shadow Run V - instant cast - Permanent 継続 - 25% detection bonus",
            "Level 35 - Shadow Run VI - instant cast - Permanent 継続 - 30% detection bonus",
            "Level 40 - Shadow Run VII - instant cast - Permanent 継続 - 35% detection bonus. 自動的に Shadow Run I を得る。ステルス状態で通常移動速度の 70%",
            "Level 45 - Shadow Run VIII - instant cast - Permanent 継続 - 40% detection bonus. 自動的に Shadow Run II を得る。ステルス状態で通常移動速度の 85%",
            "Level 50 - Shadow Run IX - instant cast - Permanent 継続 - 45% detection bonus. 自動的に Shadow Run III を得る。ステルス状態で通常移動速度の 100%"],
     text "Detect Hidden と Assassinate アビリティーは除去される。",
     text "Save Fall V はレベル 50 から 48 になる。",
     br [] [],
     text "Overshadow がレベル 49 に追加される。",
     ulist ["味方を一人ステルスにする - 10秒間継続 - 味方は非戦闘状態 - Instant cast - 1000 range - 自分には不可 - 味方は移動できるが戦闘状態になるとステルスを失う"],
     text "Vanish I をレベル 49 で習得する。",
     br [] [],
     text "Blur がレベル50 に追加される。",
     ulist ["ターゲットの場所に移動する - プレイヤーであること(敵味方は問わない) - 1000 range - 90s 再使用 - root/snare 状態でも可能だが CC は維持される - ステルスかどうかに関わらず使用可能"],
     secalb "Envenom (スペック)",
     text "毒は武器に適用するアイテムではなくなった。",
     text "毒は Envenom スペックで習得する offensive proc buff である。",
     text "この offensive proc は 100% 発動しレジストできない。",
     text "この offensive proc buff はステルス中でも詠唱可能でステルス状態を維持する。",
     text "poison proc buff を持った状態で2つの武器を使用しても発動するのは一度だけである。",
     text "ただし別のターゲットに対して毒を適用することはできる。",
     text "同一ターゲットに二重に毒を適用することはできない。",
     br [] [],
     text "poison proc buff には以下の 6 種類ある。",
     ulist ["Effectiveness poison: melee dps + attack speed debuff",
            "Withering poison: disease + melee resistance debuff",
            "Stat debuff poison: weapon skill + all stats debuff",
            "Damaging poison: DoT",
            "Shadowbind poison: snare",
            "Tranquilizing poison: mesmerization"],
     text "Shadowbind と Tranquilizing 以外の毒の再使用タイマーは 8 秒である。",
     text "Shadowbind と Tranquilizing は独立したタイマーを持ち、それぞれ 8 秒と 25 秒である。",
     text "mezz poison は Spymaster から除去される。",
     br [] [],
     text "Effectiveness Poison: ターゲットの物理攻撃ダメージと攻撃スピードの debuff. 20 秒継続。",
     ulist ["3 Envenom - Minor Essence of Lethargy. melee damage 4% attack speed 5%.",
            "13 Envenom - Weak Essence of Lethargy. melee damage 6% attack speed 10%.",
            "23 Envenom - Essence of Lethargy. melee damage 10% attack speed 15%.",
            "33 Envenom - Major Essence of Lethargy. melee damage 15% attack speed 20%.",
            "43 Envenom - Swordbreaker. melee damage 20% attack speed by 25%."],
     text "Withering Poison: disease. 15 秒継続。",
     ulist ["4 Envenom - Minor Widow Sting. Diseased, 15% slow strength 5",
            "16 Envenom - Widow Sting. Diseased, 15% slow strength 10.",
            "26 Envenom - Widow Toxin. Diseased, 15% slow, strength 15, melee resistance 5%.",
            "36 Envenom - Widow Toxin. Diseased, 15% slow, strength 20, melee resistance 10%.",
            "46 Envenom - Widow Venom. Diseased, 15% slow, strength 25, melee resistance 20%."],
     text "Stat debuff Poison: Weaponskill, Dexterity, Strength, Constitution debuff. 30 秒間継続。",
     ulist ["7 Envenom - Weakening Poison. WS 5%, stat 13.",
            "17 Envenom - Inhibiting Poison. WS 8%, stat 24.",
            "27 Envenom - Enervating Poison. WS 10%, stat 30.",
            "37 Envenom - Unnerving Poison. WS 14%, stat 41.",
            "47 Envenom - Touch of Death. WS 19%, stat 60."],
     text "Snare Poison: snare poison には root/snare タイマーが適用されない。再使用すると上書きされる。",
     ulist ["9 Envenom - Crippling Toxin. Snare 15%, 4 秒間",
            "29 Envenom - Snaring Toxin. Snare 35%, 6 秒間",
            "49 Envenom - Shadowbind. Snare 60%, 9 秒間"],
     text "Mez Poison: この poison は bodyguard を妨害できる。半径 500 内のすべてのターゲットに有効で mezz 時間短縮バフの影響を受けない。",
     ulist ["18 Envenom - Tranquilizing Gas - 5秒間継続 PBAoE mez.",
            "46 Envenom - Tranquilizing Miasma - 15秒間継続 PBAoE mez."],
     text "Damage-over-Time Poison: 20 秒間継続。",
     ulist ["1 Envenom - Minor Lethal Poison. 9 body damage, 3.9 秒ごと",
            "5 Envenom - Lesser Lethal Poison. 15 body damage every 3.9 seconds.",
            "10 Envenom - Lethal Poison. 22 body damage every 3.9 seconds.",
            "15 Envenom - Major Lethal Poison. 29 body damage 3.9 seconds.",
            "20 Envenom - Greater Lethal Poison. 36 body damage 3.9 seconds.",
            "25 Envenom - Minor Lethal Venom. 36 body damage, 22 matter damage, 3.9 秒ごと",
            "30 Envenom - Lesser Lethal Venom. 38 body damage, 30 matter damage, 3.9 秒ごと",
            "35 Envenom - Major Lethal Venom. 42 body damage, 39 matter damage, 3.9 秒ごと",
            "40 Envenom - Greater Lethal Venom. 50 body damage, 46 matter damage, 3.9 秒ごと",
            "45 Envenom - Insidious Lethal Venom. 72 body damage, 55 matter damage, 3.9 秒ごと",
            "50 Envenom - Lifebane. 101 body damage, 60 matter damage, 3.9 秒ごと"],
     secalb "Critical Strike (スペック)",
     text "Armor Wither はレジスト不可になる。",
     ulist ["Level 8 - Pincer - Side - 1% ABS debuff ではなく 4s melee stun",
            "Level 12 - Hamstring - Evade - bleed ではなく 20% attack speed debuff",
            "Level 18 - Garrote - Anytime - snare ではなく 14 damage bleed",
            "Level 21 - Perforate Artery - Frontal stealth - スタイルダメージ増加, ダメージ増加, 10秒間継続, armor wither",
            "Level 25 - Achilles Heel - rear - Attack speed debuff ではなく 10秒間継続 armor wither, damage やや軽減",
            "Level 29 - Leaper - Pincer - Bleed effect ではなく 1% ABS debuff, ダメージ増加",
            "Level 39 - Stunning Stab - Creeping Death - Bleed effect ではなく 3% ABS debuff",
            "Level 44 - Rib Separation - Achilles Heel - Armor wither ではなく 6s melee stun",
            "Level 45 - Incapacitate style は除去される。",
            "Level 46 - Neck Shot - anytime - ダメージやや軽減, helm armor slot",
            "Level 47 - Rib Shot - anytime - ダメージやや軽減, chest armor slot",
            "Level 48 - Hip Shot - anytime - ダメージやや軽減, leggings armor slot",
            "Level 50 - Ripper - Garrote - armor wither, 20秒間継続 "],
     secalb "Dual Wield (スペック)",
     ulist ["Level 21 - Penumbra - Rear - スネアはなくなる, 26% attack speed debuff, スタイルダメージ増加",
            "Level 34 - Dark Tendrils - Anytime - スタイルダメージ減少"],
     secalb "Slash (スペック)",
     ulist ["Level 39 - Backslash - Rear - スネアはなくなる, スタイルダメージ増加"],
     secalb "Thrust (スペック)",
     ulist ["Level 29 - Pierce - Rear - スネアはなくなる, スタイルダメージ増加"]]
